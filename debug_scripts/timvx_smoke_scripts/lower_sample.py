...
def lower(p: dict, input_path: Path) -> None:
    print(f"[1/6] tosa-const-fold + tosa-fold-avgpool-reduce + canonicalize "
          f"+ tosa-to-timvx + timvx-quant-residual-fuse + canonicalize "
          f"+ timvx-conv1x1-to-fc -> {p['timvx_mlir']}")
    # The WHCN boundary is op-local: each spatial-op converter wraps its
    # `timvx.conv2d` / `timvx.pool2d` in NHWC<->WHCN `timvx.transpose`
    # pairs. The rest of the IR stays in MLIR-row-major NHWC, so TOSA
    # verifiers are satisfied at every pipeline boundary.
