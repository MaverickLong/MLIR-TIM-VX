...
// will actually compile and run on this chip (VIP9000, A733).
//
...
// Softmax: official TIM-VX test uses 2D `{3, 1}` with axis=0 (innermost).
// rank-1 inputs do not appear in any TIM-VX test and the NN-engine softmax
// kernel registers its op_check on at least 2D — using rank-1 here would
// COMPILE_FAIL even though the kernel exists.
Result probe_softmax(Dt d) {
  return probe([&](std::shared_ptr<tim::vx::Context> ctx) {
    auto g = ctx->CreateGraph();
    tim::vx::ShapeType sh{3, 1};
    auto a = g->CreateTensor(make_spec(d, sh, TA::INPUT));
    auto c = g->CreateTensor(make_spec(d, sh, TA::OUTPUT));
    auto buf = filler(d, 3);
    a->CopyDataToTensor(buf.data(), buf.size());
    auto op = g->CreateOperation<tim::vx::ops::Softmax>(/*beta=*/1.0f,
                                                        /*axis=*/0);
    (*op).BindInputs({a}).BindOutputs({c});
    return g;
  });
}
// LogSoftmax: official TIM-VX test uses 2D `{6, 1}` with axis=0.
Result probe_logsoftmax(Dt d) {
  return probe([&](std::shared_ptr<tim::vx::Context> ctx) {
    auto g = ctx->CreateGraph();
    tim::vx::ShapeType sh{6, 1};
    auto a = g->CreateTensor(make_spec(d, sh, TA::INPUT));
    auto c = g->CreateTensor(make_spec(d, sh, TA::OUTPUT));
    auto buf = filler(d, 6);
    a->CopyDataToTensor(buf.data(), buf.size());
    auto op = g->CreateOperation<tim::vx::ops::LogSoftmax>(/*axis=*/0,
                                                           /*beta=*/1.0f);
    (*op).BindInputs({a}).BindOutputs({c});
    return g;
  });
}
// Topk emits two outputs: values (same dtype as input) and indices (INT32).
// Official TIM-VX test uses 2D input `{4, 3}` with values/indices flattened
// to `{k * outer} = {6}`; the kernel's `--last dimension--` doc means the
// innermost (axis=0) dim. A rank-1 input with axis=0 has no batch dimension
// to flatten over and the kernel rejects it at op_setup.
Result probe_topk(Dt d) {
  return probe([&](std::shared_ptr<tim::vx::Context> ctx) {
    auto g = ctx->CreateGraph();
    tim::vx::ShapeType in_sh{4, 3};
    tim::vx::ShapeType out_sh{6};                 // k=2 * outer=3
    auto a = g->CreateTensor(make_spec(d, in_sh, TA::INPUT));
    auto values = g->CreateTensor(make_spec(d, out_sh, TA::OUTPUT));
    auto indices = g->CreateTensor(
        tim::vx::TensorSpec(DT::INT32, out_sh, TA::OUTPUT));
    auto buf = filler(d, 12);
    a->CopyDataToTensor(buf.data(), buf.size());
    auto op = g->CreateOperation<tim::vx::ops::Topk>(/*k=*/2u);
    (*op).BindInputs({a}).BindOutputs({values, indices});
    return g;
  });
}
// Resize on a 4D WHCN image. The (align_corners=false, half_pixel_centers=
// false) combination is NOT exercised by any TIM-VX test and the chip
// appears not to register a kernel for it — every official test sets
// exactly one of the two to true. We pick (false, true), matching the
// TF2-default behavior used in production preprocessing (see the resize
// stage in timvx_ppu_npu_concurrency.cpp's preproc graph, which compiles
// and runs PASS on u8).
Result probe_resize(Dt d) {
  return probe([&](std::shared_ptr<tim::vx::Context> ctx) {
    auto g = ctx->CreateGraph();
    tim::vx::ShapeType in_sh{2, 2, 1, 1};   // W H C N inner-first
    tim::vx::ShapeType out_sh{4, 4, 1, 1};
    auto a = g->CreateTensor(make_spec(d, in_sh, TA::INPUT));
    auto c = g->CreateTensor(make_spec(d, out_sh, TA::OUTPUT));
    auto buf = filler(d, 4);
    a->CopyDataToTensor(buf.data(), buf.size());
    auto op = g->CreateOperation<tim::vx::ops::Resize>(
